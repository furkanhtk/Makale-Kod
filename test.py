...
if __name__ == "__main__":
    ...
    #raw_data = np.genfromtxt('PowerPattern.csv', delimiter=',')

    #raw_data=raw_data[:, 3]

    print("rawdata length: {}".format(len(raw_data)))
    ...
    parameter_measurement = models.get_parameter(session, id_list[33])
    # add_parameter(session,"1.8 GHz",-10,128,0,93,"Patch","Measurement")
    add_results(session,id_list[-1],raw_data,beamwidth_value[0],bandwidth_6dB_value[0],gain,tai_pereira[0],kraus[0])
    parameters_list = get_parameters(session)
    for parameter in parameters_list:
        print("id:{} F:{} P:{} D:{} mode : {} ".format(parameter.id, parameter.input_frequency, parameter.input_Power, parameter.date,parameter.mode))